...
order, num_trades, neg_sum, pos_sum = 0, 0, 0, 0
while order < len(exec_orders.values):
    ...
    while shares_left > 0:
        if exec_orders.values[order][0] == 'Sell' and order_type == 'Buy': # assuming order_type is of 'Buy', A.K.A. 'Going Long'
            n = round(exec_orders.values[order][2]*(exec_orders.values[order][3] - price_filled), 3)
            completed['net_pl'].append(n) # A ( C - B)
            if n < 0:
                neg_sum += n
            else: # n > 0
                pos_sum += n
        if exec_orders.values[order][0] == 'Buy' and order_type == 'Sell': # assuming order_type is of 'Sell', A.K.A. 'Shorting'
            n = round(exec_orders.values[order][2]*(price_filled - exec_orders.values[order][3]), 3)
            completed['net_pl'].append(n) # A ( B - C)
            if n < 0:
                neg_sum += n
            else: # n > 0
                pos_sum += n
...
net_profit = 0
completed['gross_pl'], completed['commissioned_pl'], completed['num_trades'], completed['neg_sum'], completed['pos_sum'] = [], [], [], [], []
...
for sum in completed['net_pl']:
    ...
    completed['neg_sum'].append(0)
    completed['pos_sum'].append(0)
...
completed['neg_sum'][0] = neg_sum
completed['pos_sum'][0] = pos_sum
...
completed = pandas.DataFrame(completed, columns=["order_type", "ticker", "size", "price_filled", "route", "time_filled", "net_pl", 'gross_pl', 'commissioned_pl', 'num_trades', 'neg_sum', 'pos_sum'])
